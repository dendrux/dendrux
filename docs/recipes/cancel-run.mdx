...
A complete, runnable example. A run pauses for approval; instead of approving, cancel it:
...
async def main() -> None:
    async with Agent(
        ...
        paused = await agent.run("Refund order 42 using the refund tool.")
        print(paused.status.value)                       # waiting_approval

        cancelled = await agent.cancel_run(paused.run_id)
        print(cancelled.status.value)                    # cancelled

        # Idempotent: cancelling a terminal run is a no-op, never raises.
        print((await agent.cancel_run(paused.run_id)).status.value)  # cancelled
...
Tool calls are **not preempted**, and for a non-streamed `agent.run()` neither is the in-flight LLM call. Cancellation is observed *between steps*. An interrupted tool can leave external systems in a partial state, so cooperative beats preemptive there. See [Cancellation](/docs/architecture/cancellation) for the design tradeoffs.
...
The returned `RunResult` is a receipt: `status` is still `running` and `meta["cancel_requested"]` is `True`. Confirmation is the terminal event (or `status == "cancelled"` from `RunStore.get_run`, which also exposes `cancel_requested`).

### Streamed run on this `Agent` instance

If the run was started with `stream()` or `resume_stream()` on the same `Agent` object, `cancel_run` also fires an in-process interrupt. The loop abandons the in-flight model stream immediately, even while waiting for the next token, closes the provider connection, and yields `RUN_CANCELLED` whose `answer` is the text streamed so far:

```python
async with agent.stream("Draft the announcement") as stream:
    consumer = asyncio.create_task(read_events(stream))   # forwards TEXT_DELTA to the UI

    await user_pressed_stop.wait()
    await agent.cancel_run(stream.run_id)

    events = await consumer
    final = events[-1]                       # RunEventType.RUN_CANCELLED
    kept = final.run_result.answer           # partial text, also persisted as the run's answer
    assert final.run_result.meta["interrupted"] is True
```

`run.cancelled` then carries `{"reason": "cancel_requested", "interrupted": true, "partial_output": true}`. If the model's final event had already arrived when the cancel landed, the run completes as `success` instead: a finished answer is never thrown away.

The interrupt only reaches streams driven by this `Agent` instance in this process. From another process, `cancel_run` sets the durable flag and the run cancels at its next checkpoint. Route interruption requests to the server that holds the stream.

Closing the stream yourself (`aclose()` / leaving `async with`) also ends the run as `cancelled` and persists the partial answer, with `{"reason": "stream_closed"}`.
...
`run.cancelled` carries `{"reason": "cancel_requested"}`, plus `"interrupted": true` and `"partial_output": true|false` when a streamed run was interrupted mid-generation, or `{"reason": "stream_closed", "partial_output": ...}` when the consumer closed the stream. Other terminal events (`run.completed`, `run.error`) close the conversation just the same. Refetch run detail to read the partial `answer`.
...
- **Interrupted model calls report no usage.** Providers send token counts at the end of a stream. A call abandoned mid-stream contributes nothing to `usage`; only completed iterations are counted.
