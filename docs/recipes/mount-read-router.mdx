...
If a `redact=` callback is configured on the agent, `content` is the redacted text. The unredacted execution state (`pause_data`) is never exposed by this endpoint. See [PII redaction](/docs/architecture/pii-redaction) for the full redaction boundary.
