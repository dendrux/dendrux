...
| --- | --- | --- |
| [Guardrails](/docs/architecture/guardrails) | Scan every message before and after the LLM. Built-in detectors for [PII](/docs/architecture/pii-redaction), secrets, and [prompt injection](/docs/recipes/prompt-injection-patterns), plus custom regex patterns. Redact, block, or warn. | `dendrux.guardrails` |
...
All 14 governance event types, pulled directly from `dendrux.types.GovernanceEventType`:
...
`guardrail.detected` and `guardrail.blocked` carry the matched entity type and snippet in their payload, so a single set of event types covers PII, secrets, prompt injection, and any custom guardrail without per-detector event proliferation.
...
## Producing and reading a governance event

Turn on a guardrail and the governance events land on the same `run_events` stream you read with `RunStore.get_events`. Here a PII guardrail redacts an email from the incoming message:
...
async def main() -> None:
    url = "sqlite+aiosqlite:///gov.db"

    async with Agent(
        name="Support",
        provider=AnthropicProvider(model="claude-haiku-4-5"),
        prompt="You are a support agent. Reply briefly.",
        guardrails=[PII(engine="presidio")],   # redact PII before it reaches the model
        database_url=url,
    ) as agent:
        result = await agent.run("My email is jane.doe@example.com, can you confirm it?")

    store = RunStore.from_database_url(url)
    async with store:
        for e in await store.get_events(result.run_id):
            print(e.sequence_index, e.event_type)
        # 0 run.started
        # 1 guardrail.detected
        # 2 guardrail.redacted
        # 3 llm.completed
        # 4 run.completed
...
Lifecycle events (`run.started`, `llm.completed`, `run.completed`) and governance events (`guardrail.detected`, `guardrail.redacted`) share one ordered stream. Adding `require_approval=[...]` puts `approval.requested` / `approval.decided` on the same stream; `deny=[...]` adds `policy.denied`. See [Approval](/docs/architecture/approval) and [Access control](/docs/architecture/access-control) for runnable examples of those.
