...
An admin agent has two tools: `refund` and `delete_user`. `delete_user` is in the deny list. The user asks the agent to delete user 42:
...
async def main() -> None:
    async with Agent(
        ...
        prompt="You are an admin agent.",
        tools=[refund, delete_user],
        deny=["delete_user"],                 # delete_user can never execute
        database_url="sqlite+aiosqlite:///admin.db",
    ) as agent:
        result = await agent.run("Delete user 42 using the delete_user tool.")
        print(result.status.value)            # success  (deny is a gate, not a run-killer)
        print(result.answer)                  # "...delete_user has been denied by policy..."


asyncio.run(main())
```

Captured from a live run of that script:
