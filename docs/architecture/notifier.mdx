...
Wire the `CaptureNotifier` from above into a two-iteration ReAct query and print its log:
...
async def main() -> None:
    capture = CaptureNotifier()             # the BaseNotifier subclass shown above
    async with Agent(
        name="calculator",
        provider=AnthropicProvider(model="claude-haiku-4-5"),
        prompt="You are a calculator. Use the add tool.",
        ...
        await agent.run("What is 15 + 27?", notifier=capture)   # notifier is per-call
