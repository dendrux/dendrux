...
`MCPSource.http()` and `MCPServer()` expose per-source `follow_redirects`
(default `True`), `max_redirects` (default `20`), and `destination_policy`
(default `None`). Set `follow_redirects=False` to stop at the first redirect;
there is no connection to its target. The SDK can report the unfollowed response
as a connection error during initialization.

All configured headers, including custom API-key headers and headers supplied by
credential providers, are scoped to the source's original scheme, hostname, and
effective port. Authentication-generated headers, Authorization, and Cookie are
also stripped before requests to another origin. This applies on every redirect
hop and authentication retry. Same-origin redirects retain credentials; even an
HTTP-to-HTTPS upgrade is a different origin. Redirect URLs containing userinfo
are rejected. MCP 2.2 or newer follows only method-preserving redirects within
the endpoint origin (also allowing the SDK's default-port HTTP-to-HTTPS upgrade).
Other cross-origin redirects are blocked before contacting the target; configure
that target as a separate source with its own credentials. The blocked target
is available as `MCPConnectionError.redirect_target`, while `destination` names
the origin actually contacted.

Applications own the destination policy. Dendrux calls the async policy before
opening each TCP socket, with an immutable `MCPDestination(host, port, ip)`:

```python
from dendrux.mcp import MCPDestination, MCPSource


async def destination_policy(destination: MCPDestination) -> bool:
    # Example baseline; extend with your application's network and tenant rules.
    return destination.port == 443 and destination.ip.is_global
...
Only an explicit `True` permits an address. Denied candidates are skipped;
callback exceptions abort the connection without exposing their text. If no
candidate is permitted, connection and discovery raise
`MCPDestinationDeniedError`, a subclass of `MCPConnectionError`.

Dendrux resolves each new connection, checks each candidate before attempting
it, and dials the approved numeric IP without resolving the hostname again.
IPv4-mapped IPv6 addresses are normalized to IPv4 before policy evaluation and
dialing. The original hostname remains the HTTP Host and TLS verification name.
DNS, policy evaluation, and TCP attempts share the connect timeout. Redirect
targets, new sockets after disconnect, and runtime reconnects use the same path.

With a policy configured, environment proxies and environment TLS overrides
are disabled; connections go directly to approved destinations using normal
certificate verification. There is no implicit private-network deny list when
`destination_policy=None`. The hook controls Dendrux's MCP HTTP connections;
HTTP clients independently created inside application credential callbacks or
custom authentication implementations remain application-owned.

Existing pooled sockets retain the approval from when they were opened. To
change or revoke policy for a live source, call `await runtime.rebind(...)` to
retire the existing connection and install the replacement policy. Use force
mode when active calls must be interrupted. Do not mutate a policy in place
and expect it to close already-open sockets. Each distinct policy object is
part of the runtime's connection configuration.
...
runtime = MCPRuntime(
    ...
    close_timeout=2.0,         # grace for one transport to close before cleanup goes to the background
...
Use a stable non-secret `credential_identity` (a credential row id or rotation
counter) to distinguish credential versions. `bind()` still rejects changes to
live configuration. For routine HTTP credential rotation, use:

```python
connection = await runtime.rotate_credentials(
    connection_key="github-1",
    tenant_key=tenant_id,
    credentials=TenantTokens(tenant_id),
    credential_identity="version-2",
    timeout=10.0,
    preserve_catalog=False,
)
```

Rotation pauses new calls, drains admitted calls, closes the old transport, and
reconnects lazily on the next call or discovery. Existing handles and leased
views remain usable. The drain timeout defaults to `connection_wait_timeout`;
if it expires, the old transport and credentials remain intact. Cancellation of
one waiting caller does not cancel shared rotation. A close failure prevents a
replacement from being published and requires reacquiring a connection.

The previous catalog stays in memory. By default it is revalidated after the
new handshake, and existing executors reject changed or removed tool definitions
before sending. Set `preserve_catalog=True` only when you can assert unchanged
principal and permissions: it skips `tools/list` after reconnect. Changes to
endpoint, source headers, transport policy, or other configuration still use
`rebind()`.

### Recovering from expired access tokens

Opt in with `MCPSource.http(..., reauthenticate_on_401=True)` and a provider
implementing the public `MCPRefreshingCredentialProvider` protocol:

```python
class RefreshingTokens(TenantTokens):
    async def on_auth_rejected(self) -> None:
        await invalidate_cached_access_token(self.tenant_id)  # application-owned
```

After a correlated HTTP 401 rejects a tool POST at the source origin, the runtime
drains other calls, invokes `on_auth_rejected()`, and opens a fresh transport
through `get_auth()`.
Concurrent rejections of the same credential generation share one recovery.
The rejected call is retried at most once; a second 401 raises
`MCPAuthenticationError`. 403s, background SSE failures, arbitrary tool error
text, timeouts, and uncertain outcomes do not trigger replay. Recovery
revalidates the catalog before retrying. The existing destination and credential
origin policies apply to the replacement transport.

`on_auth_rejected()` has a `connect_timeout` budget. Callback exceptions become
value-free `MCPCredentialError`s. A failed rejection callback is shared for that
credential generation; explicitly rotate credentials or rebind to try again.
One logical call emits one start and one terminal event, even if it recovers;
those events retain the first attempt's connection instance ID. Physical
rotation emits a connection-close event with `reason="credentials_rotated"`.

Dendrux does not implement OAuth exchanges, persist refresh tokens, or coordinate
rotation across workers. The provider owns those operations. Without the
opt-in flag, a live 401 is typed but is not automatically recovered.

### Safe connection diagnostics

Adapter-generated `MCPConnectionError` messages contain no URLs. Optional
`origin`, `destination`, and `redirect_target` attributes are `MCPOrigin` values
with only `scheme`, `host`, and effective `port`; `error.host` is a convenience
for the attempted host. They exclude userinfo, paths, queries, and fragments.
Origins are omitted from their default representation and from runtime events.
Hostnames can still identify private infrastructure, so persist these fields
only under your application's diagnostic policy. Diagnostics unavailable for a
particular failure remain `None`.

`transport_detail` and wrapped transport tool-error messages remove URLs as well
as configured secrets. Real server-authored tool results retain the existing
credential-redaction behavior. `MCPAuthenticationError.status_code` identifies
401/403; `request_rejected=True` identifies an HTTP rejection attributed to the
specific tool POST at the original source origin rather than an unrelated
response. A cross-origin redirect challenge is typed but never refreshes the
original source's credentials.
...
- **Rejected credentials** (HTTP 401 or 403 from the server) raise `MCPAuthenticationError`, a subclass of `MCPConnectionError` carrying `status_code`, so an application can tell "your token was rejected" from "the URL is wrong" without parsing transport text. The SDK itself reports a rejected handshake as a generic JSON-RPC error; Dendrux classifies it from the HTTP response it observed.
...
To change a live connection's configuration in one step — a rotated token, a new endpoint, a different timeout — use `rebind()`, the settings-save path:

```python
connection = await runtime.rebind(
    connection_key="github-1",
    tenant_key=user_id,
    source=MCPSource.http("github", new_url, call_timeout=30.0),
    credentials=UserToken(user_id),
)
```

`rebind()` validates the new arguments, drains (or with `mode="force"` interrupts) the live connection, forgets its registration and circuit, and binds the new configuration. Handles issued before the call are stale; use the returned one. When nothing is live it is simply a bind. Every `MCPSource` field except `name` counts as configuration — endpoint, headers, `connect_timeout`, `call_timeout`, `max_result_bytes`, and `failure_mode` — so a plain `bind()` with any of them changed raises `MCPBindingConflictError` while the connection is live.
...
`close()` stops new leases and calls, waits up to `shutdown_timeout` for active work to drain, force-evicts whatever remains, and closes every transport. It is idempotent and safe to call concurrently. Each transport gets `close_timeout` to close; a Streamable HTTP close is a network round trip because the SDK sends a session-terminating `DELETE`, which is why the default is seconds rather than milliseconds. A transport still closing after that is abandoned to background cleanup (logged as "exceeded the shutdown grace") rather than hanging your shutdown. The same grace applies to eviction and idle retirement. `MCPRuntime` is also an async context manager (`async with MCPRuntime() as runtime:`) when your application frame is a single scope.
...
A settings page needs "test this connection and show me its tools" before any chat exists. `discover()` on a managed connection does exactly that:

```python
connection = runtime.bind(connection_key="github-1", tenant_key=user_id, source=source, credentials=UserToken(user_id))
discovery = await connection.discover()

discovery.server_name, discovery.server_version, discovery.protocol_version, discovery.instructions
for tool in discovery.tools:          # raw server names, before namespacing or policy
    tool.name, tool.description, tool.input_schema, tool.annotations
```

It takes the same path an Agent lease takes — credential resolution, circuit breaker, capacity, single-flight connect — and raises the same errors, so `MCPAuthenticationError` here means the same thing it means at run time. The lease is released before it returns, which leaves the connection warm for `idle_timeout`: the next Agent reuses it without reconnecting. Calling `discover()` ahead of a user's first turn is therefore also the warm-up for first-connect latency, including after idle retirement. The returned catalog is the live physical connection's; to force rediscovery, evict.
...
`mcp.error` carries `source_name`, `namespace`, `error`, `error_type` (the exception class, such as `MCPAuthenticationError` or `MCPCircuitOpenError`), and `failure_mode`. For a `best_effort` run the same facts are on the result, so an application can tell the user which optional tools were missing for an answer:

```python
result = await agent.run("...")
for skipped in result.meta.get("mcp_skipped_sources", []):
    skipped["source_name"], skipped["error_type"], skipped["error"]
```

Discovery metadata is a snapshot taken before execution, including resumed execution.
`mcp_skipped_sources: []` means discovery completed with nothing skipped. An absent
key means the outcome is unavailable (for example, initialization failed before
discovery completed, or an idempotent result was reconstructed from storage).
Error handling never repeats discovery to populate this metadata.

Stream error events include the snapshot in `event.run_result.meta`. After a
terminal event or client closure, read `stream.result`; closure does not yield
an additional event. The `on_run_finished(run_id, result)` lifecycle hook also
receives the cancellation result with the same metadata:
...
if stream.result is not None:
    skipped = stream.result.meta.get("mcp_skipped_sources")
```

Blocking execution failures preserve their original exception type and identity.
Once execution has entered the runner's error-handling scope, the exception
exposes `run_id` and `run_meta`, including the discovery snapshot when available:
...
try:
    ...
    meta = getattr(error, "run_meta", {})
    skipped = meta.get("mcp_skipped_sources")
```

Pre-execution validation errors can occur before a run exists and therefore
need not have these attributes. `stream.result` is `None` before a terminal
outcome, including a stream closed before its first iteration.

An Agent caches discovery for its lifetime, so a source skipped once stays skipped for that Agent until `agent.refresh()`; a fresh Agent retries.
...
| `MCPDestinationDeniedError` | Destination policy denied a connection or a redirect URL was rejected. Subclass of `MCPConnectionError`. | No — correct the destination or policy. |
| `MCPAuthenticationError` | The server rejected the source's credentials (HTTP 401/403). Subclass of `MCPConnectionError`; carries `status_code`. | No — refresh or rotate the credential, then evict and rebind. |
