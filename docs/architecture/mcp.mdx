...
are rejected. Cross-origin redirects may therefore fail authentication at the
target; configure that target as a separate source with its own credentials.
