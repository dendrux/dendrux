---
title: Guardrails
description: Pattern-based content scanners that redact, block, or warn on PII, secrets, prompt injection, and custom rules, applied before and after every LLM turn.
---
...
async def main() -> None:
    url = "sqlite+aiosqlite:///secrets.db"
    ...
    async with Agent(
        ...
        result = await agent.run("Here is my key: AKIAIOSFODNN7EXAMPLE, please store it.")
        print(result.status.value)        # error  (the secret never reaches the model)
        print(result.error)
        # Guardrail 'SecretDetection' blocked: AWS_ACCESS_KEY matched 'AKIAIOSFODNN7EXAMPLE'

    store = RunStore.from_database_url(url)
    async with store:
        print([e.event_type for e in await store.get_events(result.run_id)])
        # ['run.started', 'guardrail.blocked', 'run.completed']
...
The captured governance rows from that run:
...
1. **User input + tool-result re-entry** — the engine's incoming scan walks every message handed to `provider.complete()` on each iteration. Tool result messages are part of the prompt on the next turn, so the same scan catches injection arriving via web fetches, file reads, search results, and MCP outputs.
2. **LLM output + tool-call params** — the outgoing scan catches a model that complies with an earlier injection and tries to act on it.
