...
A complete example: write a `SKILL.md`, load it with `skills_dir=`, run a prompt that triggers it, and read the skill events back:
...
# 1. A skill on disk: _skills/json-formatter/SKILL.md
skill_dir = Path("_skills/json-formatter")
...
(skill_dir / "SKILL.md").write_text(
    ...
    "---\n\n"
    "# JSON Formatter\n\n"
    "When asked to format JSON, parse it and re-emit with 2-space indent.\n"
)


async def main() -> None:
    url = "sqlite+aiosqlite:///skills.db"
    ...
    async with Agent(
        ...
        result = await agent.run('Please format this JSON for me: {"b": 2, "a": 1}')

    store = RunStore.from_database_url(url)
    async with store:
        print([e.event_type for e in await store.get_events(result.run_id)])
        # ['run.started', 'skill.registered', 'llm.completed', 'tool.completed',
        #  'skill.invoked', 'llm.completed', 'run.completed']


asyncio.run(main())
```

The captured timeline below adds a second, denied skill (`deny_skills=["dangerous-tool"]`) to also show `skill.denied`:
