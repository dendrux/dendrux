...
**Cancellation is cooperative, with one controlled exception.** When you call `cancel_run`, dendrux sets a flag in the database. The runner observes that flag at two safe checkpoints: the top of every iteration, and right before pausing. In-flight tool calls always finish first. For a streamed run owned by the same `Agent` instance, `cancel_run` also interrupts the in-flight model stream between tokens and keeps the partial text as the run's `answer`, so a one-turn chat reply can be stopped mid-sentence. Either way the run ends with a `run.cancelled` event. See [Cancellation](/docs/architecture/cancellation) for the full story.
